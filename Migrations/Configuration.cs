namespace BlackMarket_API.Migrations
{
	...
	internal sealed class Configuration : DbMigrationsConfiguration<BlackMarket_API.Data.Models.BlackMarket>
	{
		...
		protected override void Seed(BlackMarket_API.Data.Models.BlackMarket context)
		{
			...
			//Add "All categories" category at index 0
			Category allCategories = new Category { CategoryId = 0, Name = "All categories", PhotoPath = "0.png", Description = "" };

			string sqlCommand = "";

			sqlCommand += (
				$"SET IDENTITY_INSERT dbo.{nameof(Category)} ON ");

			sqlCommand += (
				$"INSERT INTO dbo.Category (CategoryId, Name, Description, PhotoPath) " +
				$"Values " +
				$"({allCategories.CategoryId}, '{allCategories.Name}', '{allCategories.Description}', '{allCategories.PhotoPath}')");

			sqlCommand += (
				$"SET IDENTITY_INSERT dbo.{nameof(Category)} OFF");
			...
			List<Product> products = new List<Product>
			{
				...
				new Product() { Name = "Relly Christmar Tree Ball", Price = 20M,  PhotoPath = "9.png", CategoryId = CategoryFrom(categories, "Accessories"), Description = "Jingle bells, jingle bells, jingle all the way..." , ExtraDescription = "New year without Christmas tree and Christmas ball? You should be kidding! Get this one, hand it on your tree and sing the songs! Even if it's summer. You have your own celebration schedule." },
